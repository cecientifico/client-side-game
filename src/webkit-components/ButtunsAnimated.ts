  ...
  constructor(els: HTMLElement[]) {
    ;(this.elementData = {
      ...
      els.forEach((item: HTMLElement) => {
        ...
        item.addEventListener('mouseup', ({ target }: any) => this.finishAnimation(target))
        item.addEventListener('mouseleave', ({ target }: any) => this.finishAnimation(target))
        item.addEventListener('touchend', ({ target }: any) => this.finishAnimation(target))
